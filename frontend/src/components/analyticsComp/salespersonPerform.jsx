...
const SalespersonPerform = ({ salespersonPerform, ...rest }) => {
  ...
  useEffect(() => {
    axios
      .get(URL.main + URL.salesPersonAnalyticsDuration + "/" + "Day-7", {
        headers: authHeader(),
      })
      .then((response) => {
  ...
  const changeSelectOptionHandler = (event) => {
    setSelected(event.target.value);
    {
      event.target.value === "Month"
        ? setSelectedValue("January")
        : event.target.value === "Day"
        ? setSelectedValue("Last 7 Days")
  ...
  if (type) {
    options = type.map((el) => (
      <option key={el} value={selected === "Month" ? type.indexOf(el) + 1 : el}>
        {el}
      </option>
    ));
  }
  const changeRenderComp = (dur) => {
    ...
    axios
      .get(URL.main + URL.salesPersonAnalyticsDuration + "/" + dur, {
        headers: authHeader(),
      })
      .then((response) => {
      ...
      <div id="salespersonPdf">
        ...
        <form style={{ margin: "0px 60px" }} onSubmit={onSubmitDuration}>
          <div className="row">
            ...
            <select
              ...
              style={{ backgroundColor: "rgba(239, 228, 228, 0.5)" }}
        ...
        {sellerPerform.length > 0 ? (
          <div>
            <ResponsiveContainer
              width="100%"
              height={75 * sellerPerform.length}
            >
              <BarChart
                data={sellerPerform}
              ...
              >
                <XAxis
                  type="number"
                  domain={[0, (dataMax) => sellerPerform[0].sales]}
                  orientation="bottom"
                  stroke="black"
                />
                <YAxis
                ...
                <Tooltip
                  ...
                  contentStyle={{ width: "150px", height: "80px" }}
                />
